...
class MDGTDTInet(nn.Module):
    def __init__(self, compound_dim=128, protein_dim=128, gt_layers=5, gt_heads=8, out_dim=1):
        ...
        self.protein_gt = gt_net_protein.GraphTransformer(device, n_layers=gt_layers, node_dim=361, edge_dim=5,
                                                          hidden_dim=protein_dim,
                                                          out_dim=protein_dim, n_heads=gt_heads, in_feat_dropout=0.0,
                                                          dropout=0.2, pos_enc_dim=8)


        self.protein_embedding_fc = nn.Linear(320, self.protein_dim)
        self.Compound_embedding_fc = nn.Linear(512, self.compound_dim)
    ...
    def forward(self, compound_graph, protein_graph,  protein_embedding, Compound_embedding):
        compound_feat = self.compound_gt(compound_graph)
        compound_feat_x = self.dgl_split(compound_graph, compound_feat)
        Compound_embedding = self.Compound_embedding_fc(Compound_embedding)
        compound_feats = self.crossAttention(Compound_embedding, compound_feat_x, compound_feat_x)
        # compound_feats = compound_feat_x
        # compound_feats = torch.cat([compound_feat_x, Compound_embedding], dim=1)
        ...
        output = self.classifier(cp_embedding)
        x = output
        # output2 = self.classifier2(cp_embedding)
        # y = output2
